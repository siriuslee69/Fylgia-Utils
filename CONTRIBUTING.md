...
- `evaluation/tests/`: smoke and focused regressions.
- `fylgia_utils.nimble`: canonical tasks (`test`, `smoke`, `runModuleTests`, `autopush`, `switch`, `applyNightly`, `find`).
- `README.md`: usage and issue playbook.
- `agents/PROGRESS.md`: progress log and commit message source.
...
5. Keep `autopush` and progress metadata paths aligned with `agents/PROGRESS.md`.

## Commands

```bash
nimble test
nimble runModuleTests
nimble runModuleTests <moduleName>
nimble smoke
```
